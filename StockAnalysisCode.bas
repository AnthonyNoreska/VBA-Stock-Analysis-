Sub StockAnalysis()
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        For i = 2 To lastrow
            ...
            ' Check if current row represents the first row of a new year
            If IsDate(ws.Cells(i, "A").Value) And IsDate(ws.Cells(i - 1, "A").Value) Then
                If Year(ws.Cells(i, "A").Value) <> Year(ws.Cells(i - 1, "A").Value) Then
                    ' Update OpenPrice with the value from the current row
        ...
        If currentMaxVolume > MaxVolume Then
            ...
            MaxVolumeTicker = TickerName ' Move this line inside the loop
        End If
